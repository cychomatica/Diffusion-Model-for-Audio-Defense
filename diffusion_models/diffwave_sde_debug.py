...
class RevVPSDE(torch.nn.Module):
    ...
    def reverse_diffusion_sampling(self, t_rev, x_t):
        
        for t in range(t_rev-1, -1, -1):

            beta_t = self.beta_0 + t / self.N * (self.beta_1 - self.beta_0)
            F_t, G_t = - beta_t / 2, beta_t**0.5

            if t > 0:
                epsilon_t = self.model.compute_eps_t(x_t, t)
                score = _extract_into_tensor(self.sqrt_1m_alphas_cumprod_neg_recip_cont, torch.tensor(t / self.N), x_t.shape).to(x_t.device) * epsilon_t

                mu_t = x_t - F_t + G_t ** 2 * score
                sigma_t = G_t
                ...
                epsilon_t = self.model.compute_eps_t(x_t, t)
                score = _extract_into_tensor(self.sqrt_1m_alphas_cumprod_neg_recip_cont, torch.tensor(t / self.N + 1e-5), x_t.shape).to(x_t.device) * epsilon_t
                mu_t = x_t - F_t + G_t ** 2 * score
...
class RevGuidedDiffusion(torch.nn.Module):
    ...
    def audio_editing_sample(self, audio):
        ...
        for it in range(self.args.sample_step):
            ...
            x0_sde_sampling = self.rev_vpsde.reverse_diffusion_sampling(self.args.t, x)
            x_ = x.view(batch_size, -1)  # (batch_size, state_size)
            # x_ = x
            # self.args.use_bm = True
            if self.args.use_bm:

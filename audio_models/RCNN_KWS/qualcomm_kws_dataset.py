...
class RandAudioLength(object):
    ...
    def __call__(self, data):
        ...
        if length < len(samples):
            ...
            data['samples'] = np.pad(samples, (0, length - len(samples)), "constant")
        return data
# folder = '/home/shutong/project/Diffusion_Model_for_Audio_Defense_Release/datasets/qualcomm_keyword_speech_dataset'
# class_name = 'hey_android'
# d = os.path.join(folder, class_name)
# for root, dirs, files in os.walk(d, topdown=False):
#     for name in files:

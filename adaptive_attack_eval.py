...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_path', default='vpsde_adaptive_pgd_untargeted')
    ...
    if args.attack == 'CW':
        Attacker = AudioAttack(model=Classifier, transform=Wave2Spect, defender=DiffWave_VPSDE, max_iter_1=args.max_iter_1, max_iter_2=0)
    ...
    for batch in pbar:
        ...
        waveforms_adv_denoised = DiffWave_VPSDE(waveforms=waveforms_adv)
        ...
        correct_adv += waveforms.shape[0] - attack_success
        ...
        pbar.set_postfix(
            {
                ...
                'whitebox robust acc: ': '{:.4f}%'.format(acc_adv),
            }
        )
        pbar.update(1)
    ...
    print('whitebox robust test accuracy: {:.4f}%'.format(acc_adv))
